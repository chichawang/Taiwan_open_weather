...
cn=ax.quiver(lon,lat,u_data,v_data,ws_data,transform=ccrs.PlateCarree(),
        zorder=10,cmap=plt.get_cmap('rainbow'),scale=200)
data=np.around(json_df['TEMP'].to_numpy().astype(np.float32),decimals=2)
data[data<-90.] = np.nan
cn=ax.scatter(lon,lat,transform=ccrs.PlateCarree(),zorder=10,s=10,cmap=plt.get_cmap('rainbow'),c=data,vmin=-10.,vmax=40.)
...
data=np.around(json_df['24R'].to_numpy().astype(np.float32),decimals=2)
data[data<-0.] = np.nan
...
ax.collections.clear()
ax.set_title('UVI')
data=np.around(json_df['H_UVI'].to_numpy().astype(np.float32),decimals=2)
data[data<-0.] = np.nan
print(np.nanmax(data),np.nanmin(data))
cn=ax.scatter(lon,lat,transform=ccrs.PlateCarree(),zorder=10,s=5,cmap=plt.get_cmap('rainbow'),c=data,vmin=0.,vmax=100.)
bar=plt.colorbar(mappable=cn,ax=ax)
#plt.colorbar(cn)
ofilename=str(filename).split('.')[0]+'_UVI.png'
print(ofilename)
plt.savefig(ofilename,dpi=200)
...
#ax.set_title('VIS')
#print(json_df['VIS'])
#json_df['VIS'].replace("無觀測",np.nan)
#data=np.around(json_df['VIS'].to_numpy().astype(np.float32),decimals=2)
#data[data<-0.] = np.nan
#print(np.nanmax(data),np.nanmin(data))
#cn=ax.scatter(lon,lat,transform=ccrs.PlateCarree(),zorder=10,s=5,cmap=plt.get_cmap('rainbow'),c=data,vmin=0.,vmax=10.)
#bar=plt.colorbar(mappable=cn,ax=ax)
##plt.colorbar(cn)
#ofilename=str(filename).split('.')[0]+'_VIS.png'
#print(ofilename)
#plt.savefig(ofilename,dpi=200)
